...
const uploadvehicle = async (req, res) => {
  try {
    ...
    if (req.body.ContractType ==='' || req.body.VehiclesType ==='' || req.body.FuelType === '' || req.body.Colour === ''|| req.body.Transmission === '' || req.body.ManufacturingYear ==='' || req.body.imageUrls ==='' || req.body.VIN ===''|| req.body.Description ===''|| req.body.Price ==='') {
      return res.status(400).json({ success: false, message: 'All fields are required.' });
  }

  if (!/^\d+$/.test(req.body.Price)) {
  return res.status(400).json({ success: false, message: 'Price only contains numbers.' });
}
if (!/^\d+$/.test(req.body.Rating)) {
  return res.status(400).json({ success: false, message: 'Rating only contains numbers.' });
}
if (!/^[a-zA-Z]+$/.test(req.body.Colour) ) {
  return res.status(400).json({ success: false, message: 'Colour must contain only letters.' });
} 
if (!/^\d+$/.test(req.body.Rating)) {
  return res.status(400).json({ success: false, message: 'Rating only contains numbers.' });
}
if (!/^\d+$/.test(req.body.ManufacturingYear)) {
  return res.status(400).json({ success: false, message: 'ManufacturingYear only contains numbers.' });
} 
if (!/^\d+$/.test(req.body.VIN)) {
  return res.status(400).json({ success: false, message: 'VIN only contains numbers.' });
}
    ...
    res.json({ success: true, message: 'vehicle information added successfully' ,
    data: {
...
const deletevehicle= async (req, res) => {
  try {
    ...
    res.json({ success: true, message: 'vehicle deleted successfully', deletedvehicle });
  } catch (error) {
    console.error('Server error:', error);
    if (!res.headersSent) {
      res.status(500).json({ success: false, error: 'Internal Server Error' });
...
const updatevehicle = async (req, res) => {
  try {
    ...
    const updatedvehicle = await Vehicle.findByIdAndUpdate(VehicleId, updatedData, { new: true });

    if (!updatedvehicle) {
      return res.status(404).json({ success: false, error: 'vehicle not found.' });
    }

    res.json({ success: true, message: 'vehicle information updated successfully', data: updatedvehicle });
  } catch (error) {
    console.error('Server error:', error);
    if (!res.headersSent) {
      res.status(500).json({ success: false, error: 'Internal Server Error' });
...
const getvehiclebyid = async (req, res, next) => {
  try {
      ...
      res.status(500).json({ success: false, message: 'An error occurred while fetching vehicle.' });
  }
};
const approveVehicle = async (req,res) =>{
  try {
    ...
    if (!isAuthorizedToApprove(user)) {
      return res.status(403).json({ success: false, error: 'User is not authorized to approve vehicle.' });
    }
...
const rejectVehicle = async (req, res) => {
  try {
    ...
    if (!isAuthorizedToReject(user)) {
      return res.status(403).json({ success: false, error: 'User is not authorized to reject houses.' });
    }
    ...
    res.json({ success: true, message: 'vehicle rejected successfully.', data: vehicle });
  } catch (error) {
...
const assignBrokerToVehicle = async (req, res) => {
  try {
    ...
    if (!isAuthorizedToAssign(broker)) {
      return res.status(403).json({ success: false, error: 'Broker is not authorized to be assigned.' });
    }

    vehicle.Broker = broker._id; 
    await vehicle.save();

    res.json({ success: true, message: 'Broker assigned successfully.', data: vehicle });
  } catch (error) {
