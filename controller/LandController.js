...
const uploadland = async (req, res) => {
  try {
    ...
    if (req.body.ContractType ==='' || req.body.Area ==='' || req.body.imageUrls ==='' || req.body.Location ==='' || req.body.City ==='' || req.body.Description ===''|| req.body.Price ==='') {
      return res.status(400).json({ success: false, message: 'All fields are required.' });
  }
    
  if (!/^\d+$/.test(req.body.Area)) {
    return res.status(400).json({ success: false, message: 'Area only contains numbers.' });
}
...
if (!/^[a-zA-Z]+$/.test(req.body.City) ) {
  return res.status(400).json({ success: false, message: 'City must contain only letters.' });
} 
    ...
    res.json({ success: true, message: 'Land information added successfully' ,
    data: {
      newland: savedland, 
    }});
  } catch (error) {
...
const deleteland= async (req, res) => {
  try {
    ...
    res.json({ success: true, message: 'Land deleted successfully', deletedland});
  } catch (error) {
    console.error('Server error:', error);
    if (!res.headersSent) {
      res.status(500).json({ success: false, error: 'Internal Server Error' });
...
const updateland = async (req, res) => {
  try {
    ...
    const updatedProperty = await Land.findByIdAndUpdate(LandId, updatedData, { new: true });
    ...
    res.json({ success: true, message: 'Land Information updated successfully', data: updatedProperty });
  } catch (error) {
    console.error('Server error:', error);
    if (!res.headersSent) {
...
const getlandbyid = async (req, res, next) => {
  try {
      ...
      res.status(500).json({ success: false, message: 'An error occurred while fetching Land.' });
  }
};
...
const approveLand = async (req,res) =>{
  try {
    ...
    if (!isAuthorizedToApprove(user)) {
      return res.status(403).json({ success: false, error: 'User is not authorized to approve land.' });
    }
    ...
    res.json({ success: true, message: 'Land approved successfully.', data: land });
  } catch (error) {
...
const rejectLand= async (req, res) => {
  try {
    ...
    if (!isAuthorizedToReject(user)) {
      return res.status(403).json({ success: false, error: 'User is not authorized to reject land.' });
    }
    ...
    res.json({ success: true, message: 'Land rejected successfully.', data: land });
  } catch (error) {
...
const assignBrokerToLand = async (req, res) => {
  try {
    ...
    res.json({ success: true, message: 'Broker assigned successfully.', data: land });
  } catch (error) {
